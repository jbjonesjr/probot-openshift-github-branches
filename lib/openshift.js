...
function createOCPresources(ocp_template) {
	openshiftConfigLoader({configLocation:'./.openshift/config.yaml'}).then(config => {
		  openshiftRestClient(config).then(client => {
		    ...
		    const createresult_service = client.services.create(ocp_template.ocp_service).then(createresult_service => {
		    ...
		    const createresult_deploy = client.deploymentconfigs.create(ocp_template.ocp_deploy).then(createresult_deploy => {
		    ...
		    const createresult_imagestream=client.imagestreams.create(ocp_template.ocp_imagestream).then(createresult_imagestream => { console.log(createresult_imagestream); } );
		    ...
		    const createresult_build = client.buildconfigs.create(ocp_template.ocp_build).then(createresult_build => {
		    ...
		    const createresult_route=client.routes.create(ocp_template.ocp_route).then(createresult_route => { console.log(createresult_route); } );
...
module.exports = {
  async pr_opened(event) {
    console.log('os pr(', event.payload.number, ') was', event.payload.action, ': ', event.payload.pull_request.title, " branch name: " , event.payload.pull_request.head.ref);
