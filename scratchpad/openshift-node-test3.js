...
ocp_myphp_build.spec.source.git.ref="devnation-canary-notok"
...
ocp_myphp_route.metadata.name = ocp_myphp_route.metadata.name + "-pullrequest-" + pullrequest_id ;
ocp_myphp_route.spec.alternateBackends[0].name=ocp_myphp_service.metadata.name;
ocp_myphp_route.spec.alternateBackends[0].weight=10
ocp_myphp_route.spec.to.weight=90
ocp_myphp_route.spec.host="pullrequest-" + pullrequest_id + "-" + ocp_myphp_route.spec.host;
...
openshiftConfigLoader({configLocation:'./.openshift/config.yaml'}).then(config => {
  openshiftRestClient(config).then(client => {
    ...
    const createresult_service = client.services.create(ocp_myphp_service).then(createresult_service => {
    ...
    const createresult_deploy = client.deploymentconfigs.create(ocp_myphp_deploy).then(createresult_deploy => {
    ...
    const createresult_build = client.buildconfigs.create(ocp_myphp_build).then(createresult_build => {
    ...
    console.log("the route to create is ", ocp_myphp_route.metadata.name );
    console.log(ocp_myphp_route);
    console.log("--> creating route...");
    const createresult_route=client.routes.create(ocp_myphp_route).then(createresult_route => { console.log(createresult_route); } );
